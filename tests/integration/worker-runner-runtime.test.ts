...
test('(c) a NonRetryableError fails the job non-retryably', async () => {
  ...
  expect(queue.fail).toHaveBeenCalledWith(job.id, 'w1', job.leaseToken, 'bad input', { retryable: false });
...
test('(e) a rejecting heartbeat is treated as lease loss — no unhandled rejection', async () => {
  ...
  try {
    ...
    expect(queue.fail).toHaveBeenCalledTimes(1);
    expect(queue.fail).toHaveBeenCalledWith(job.id, 'w1', job.leaseToken, 'aborted', { retryable: true });
...
test('(g) wall-clock exceeded aborts and fails the job retryably', async () => {
  ...
  expect(outcome).toBe('failed');
  expect(queue.fail).toHaveBeenCalledTimes(1);
  expect(queue.fail).toHaveBeenCalledWith(job.id, 'w1', job.leaseToken, 'wall clock exceeded', { retryable: true });
