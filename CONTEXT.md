...
- **Source-vs-derived test** — the split between the two blob kinds keys on **recreation cost, not lineage**: an artifact that is *derived* from a source but is only re-obtainable via a **paid, non-deterministic** call (a Gemini call) is a **source-of-truth blob**, not a derived cache — because losing it cannot be silently, freely, identically rebuilt.
- **Magazine model** — the structured, per-section `{lead, bullets}` representation the rendered HTML doc is built from, produced by a Gemini restyle of the summary (`generateMagazineModel`) and persisted as `models/<base>.json`. By the source-vs-derived test it is a **source-of-truth blob** (paid, non-deterministic to recreate), even though it is lineage-derived from the summary. Therefore a **missing** magazine model behind a *promoted* summary is **repair needed** — surfaced, never silently regenerated, and **never regenerated on the serve path**. Distinct from the **envelope**, the stored JSON wrapper (`{sourceMd, generatedAt, sourceSections, model}`) whose `model` field carries the magazine model.
