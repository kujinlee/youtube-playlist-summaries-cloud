...
| 13 | **Gated dev-login (local manual testing)** — the app login is **Google-OAuth only** (`app/login/page.tsx` → `signInWithOAuth({provider:'google'})`), and local Google OAuth needs client-id/secret env vars that are usually unset — so there is **no way to sign into the app UI against local Supabase (#2)** for manual testing. The app *has* `signInWithPassword` (used by the cloud-sync CLI); the UI just doesn't expose a form. Add a **dev-only email/password login** that renders **only** when pointed at a non-prod Supabase (gate on `NODE_ENV !== 'production'` **and** the Supabase URL being localhost — fail-closed so it structurally cannot appear in prod), **plus a guard test** asserting it is absent in prod. NOT a backdoor — a reviewed, environment-gated dev affordance (mirrors `config.toml`'s `skip_nonce_check` local-vs-prod pattern). Enables browser-based manual testing of the cloud app against #2 (with a synthetic email/password user). | `app/login/page.tsx` (or a `/dev-login` route) + env gate + guard test | S | (dev/testing) | pending (needs spec) |
