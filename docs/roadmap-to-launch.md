...
- **[backlog #37]** — a newly ingested playlist does not appear in the sidebar (3 existed, 1 listed).
  ...
  were both excluded by one query showing three rows, one owner, three non-null titles. **Filing from
  the symptom named a severity and two mechanisms before anyone opened the component**, and all three
  were wrong; only the observation "3 exist, 1 listed" survived contact with the code.
...
(backlog #36 and #37; **#37 root-caused and fixed the same day in PR #91** — its filed severity and
both of its filed causes turned out to be wrong, which is recorded with the acceptance evidence
above because *"filed from the symptom"* is a repeatable failure, not a one-off). **3.1 is a TASK, not a gate** — the test becomes the claim once written, so its
