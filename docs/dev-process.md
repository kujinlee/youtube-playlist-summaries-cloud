...
| `scripts/check-guard-coverage.py` | every guard classified SHAPE/SEQUENCE; every SEQUENCE guard reconciles and is mutated. ⟳ 2026-09-04 (Phase 6 #7, finding 4): this row claimed enforcement while `ci.yml` referenced it **zero** times — its only automated caller was `check-schema-gates.sh`, which needs a live Postgres this guard never uses. Now a CI step of its own |
