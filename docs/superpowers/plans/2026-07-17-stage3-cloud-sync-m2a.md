...
- Modify: `lib/html-doc/generate.ts:49-55` (+ any serve-time model writer) — set `sourceMdHash: mdHash(md)` in the envelope, where `md` is the MD **body** (`generate.ts:33`), NOT `sourceMd`/`video.summaryMd` (the key) — High; see T4(c)
...
```ts
// The corrections THIS regenerated MD now reflects = the value stored AFTER the conditional
// update above: param provided non-empty → trimmedCorrections; param === '' → '' (cleared);
// param ABSENT → the UNCHANGED stored value (a bare regenerate keeps prior corrections baked in).
// Stamping mdHash('') on a bare regenerate would mis-mark a still-corrected MD as stale (round-2 N5).
// Mirror the conditional update above: a write happens iff trimmedCorrections is truthy
// (→ that value) or the param is exactly '' (→ cleared to ''). Otherwise (undefined OR
// whitespace-only, which doesn't fire either branch) the stored value is unchanged (round-3 L-R3-2).
...
```
Without this, a corrected MD is judged **corrections-stale forever** and a stale higher-format uncorrected MD from the other replica can overwrite it — the exact hazard §5.3 exists to prevent. Note: `updateVideoFields` here writes MD-currency fields, NOT a Class-B field, so it must **not** bump `annotationsEditedAt` (the separate earlier `updateVideoFields({ corrections })` at line 55 is the Class-B write that stamps `annotationsEditedAt.corrections`). Confirm whether `fixSummary` re-applies stored corrections on a bare regenerate; if a future change makes a bare regenerate DROP prior corrections, revisit `effectiveCorrections`.
...
**(d) Class-B stamping in the store layer.** Local: in `local-metadata-store.ts`, **widen the `updateVideoAnnotations` allowlist Set from `{personalScore, personalNote, archived}` to `{personalScore, personalNote, archived, corrections}`** (round-2 N3 — `applyClassBWinners` writes `corrections` to a local loser through `updateVideoAnnotations`; without this the allowlist silently drops it). Both `updateVideoAnnotations` AND `updateVideoFields` (in `index-store.ts`/`local-metadata-store.ts`) stamp per-field `annotationsEditedAt` when a Class-B key (`personalNote`/`personalScore`/`corrections`) is set or cleared (user path → `now()`, sync path → `opts.editedAt`), and NOT for a non-Class-B field write. Cloud: `supabase-metadata-store.ts.updateVideoAnnotations` passes `corrections` in `p_set`/`p_clear` and forwards `p_edited_at: opts?.editedAt`; `updateVideoFields` (→ `merge_video_data`) forwards `p_edited_at` when present.
...
it('a freshly written model envelope carries sourceMdHash = mdHash(MD BODY)', async () => {
  const BODY = '# Section A\n\n- point one\n- point two\n'; // the MD body fed to generation
  // Run the real generation path with a summaryMd key whose blob holds BODY, then read the envelope:
  const env = /* readModelEnvelope(principal, base) */;
...
export function makeFileTokenStore(file: string): TokenStore {
  return {
    ...
    async write(token: string) {
      ...
      catch (e: any) { if (e?.code === 'ENOENT') exists = false; else throw; } // unsafe existing → throw
      if (!exists) {
...
| 2 | Class B before Class A | every **two-sided** video (both present) | `reconcileHuman` runs, its winners applied, `reconciledCorrectionsHash` computed, THEN `reconcileClassA`. One-sided videos are resolved by the presence branch (#3/#7/#8) and never reach this. |
| 3 | Additive create (money-safe) | video present one side, not in baseline | `ensureReceiverSlot` first (create playlist+reservation row — cloud `upsertVideo` only UPDATEs); MD blob written+verified BEFORE `promoted` status; verify receiver row exists BEFORE writing the baseline; **never** call the metered producer/enqueuer; `report.created++` |
...
- **`sanitizeAdditiveVideo(video: Video): Video`** (Behavior #3, money-safe — round-2 H3) — returns a copy of the record with **all regenerable-cache and out-of-scope pointers cleared** so the receiver never advertises artifacts whose blobs were not copied: set `summaryHtml`, `digDeeperHtml`, `digDeeperMd` to `null`/absent, and drop every `artifacts.*` entry EXCEPT `artifacts.summaryMd`. Also drop the sender's replica-local ordering (`position`, `serialNumber`, `playlistIndex`, `removedFromPlaylist`) — the receiver's `ensureReceiverSlot` claim supplies these (§4.1). **Keep** identity, Class-A scalars (`ratings`/`overallScore`/`videoType`/`audience`/`tags`/`tldr`/`takeaways`), `summaryMd` (the MD key), the md signals (`mdGeneratedAt`/`mdCorrectionsHash`), the human fields (`personalNote`/`personalScore`/`corrections`), **and their per-field `annotationsEditedAt`** (round-4 L5 — carrying the real edit timestamps means the next sync of this now-two-sided video sees the receiver as non-backfilled, avoiding needless convergence churn).
- **`playlistMetaFor(key, localPlaylists, cloudSummaries): { playlistUrl: string; playlistTitle?: string }`** — resolve the playlist's URL/title for `key` from the local registry entry (`LocalPlaylist.playlistUrl`) or the cloud `PlaylistSummary` (`playlistUrl`/`playlistTitle`), whichever holds it — used by `ensureReceiverSlot` to create the receiver playlist row.
- **`ensureReceiverSlot(to: MetadataStore, toP: Principal, playlistMeta, video: Video): Promise<{ position: number; serialNumber: number } | null>`** (round-4 H1 — REQUIRED before any receiver write) — the cloud `upsertVideo` is a bare `UPDATE` of a row pre-created by `claimVideoSlot` (`supabase-metadata-store.ts:104-113`): an update of a non-existent playlist/video row **silently affects 0 rows and does not throw**. So additive create MUST first create the receiver rows: (1) ensure the playlist exists — `to.setPlaylistMeta(toP, { playlistUrl, playlistTitle })` (idempotent upsert on `owner_id,playlist_key`); (2) if `to.readIndex(toP)` does NOT already contain `video.id`, `return await to.claimVideoSlot(toP, video.id)` to create the reservation row; if the row already exists, return `null`. (`claim_video_slot` inserts with `on conflict (playlist_id, video_id) do nothing` — a repeat call on an existing row yields no/uninformative `position`/`serialNumber`, so guard the claim behind the readIndex-absence check to only claim a genuinely new slot; single-run/no-concurrency makes the absence check authoritative.) Backend-uniform (local `setPlaylistMeta`/`claimVideoSlot` also exist). **The claimed `{ position, serialNumber }` are the RECEIVER's replica-local ordering (§4.1 — `position`/`serialNumber`/`playlistIndex` are replica-local, NOT synced); `copyAdditiveVideo` sets them on the sanitized record instead of carrying the sender's.** Without this, a local-only playlist/video published to cloud no-ops and the MD blob is orphaned.
- **`copyAdditiveVideo(to: MetadataStore, toP: Principal, toBlob: BlobStore, playlistMeta, video: Video, mdBody: string | null): Promise<void>`** (Behavior #3, money-safe) — the receiver blob store is passed **explicitly** as `toBlob` (round-3 H1). Order (round-4 H1/M1 — blob durable BEFORE promoted status):
  1. `const slot = await ensureReceiverSlot(to, toP, playlistMeta, video)` — the receiver playlist+video row now exists; `slot` (if non-null) is the receiver's replica-local `{position, serialNumber}`.
  2. If `video.summaryMd && mdBody != null`: `await toBlob.put(toP, video.summaryMd, Buffer.from(mdBody, 'utf8'), 'text/markdown')` and **verify** it is readable + hashes to `mdHash(mdBody)`. A summary-less one-sided video (ingested-not-yet-summarized) skips the blob (round-3 M-R3-1 — a `put` under a null key would throw) and does NOT mark promoted.
  3. Build `sanitized = sanitizeAdditiveVideo(video)`; if `slot`, set `sanitized.position = slot.position` and `sanitized.serialNumber = slot.serialNumber` (receiver-local ordering, §4.1); if `slot` is `null` (row already existed — not reachable on the additive path in a single non-concurrent run, round-5 L1), re-read and keep the receiver's existing `position`/`serialNumber` rather than clobbering with `undefined`. **Only if step 2 wrote+verified the blob**, set `artifacts.summaryMd = { key: video.summaryMd, status: 'promoted' }` (round-4 M1 — never advertise `promoted` before the blob is durable; L4 — direction-agnostic: cloud reads `summaryReady` off it, local ignores `artifacts` and serves the MD file). Then `await to.upsertVideo(toP, sanitized)`.
  4. **Verify** `to.readIndex(toP)` now contains `video.id` — the caller writes the baseline ONLY after this confirms the receiver row exists (round-4 H1 — never advance a manifest baseline for a copy that silently no-op'd).
  **Never** import/call `lib/job-queue/producer.ts` or any enqueue; **never** copy `summaryHtml`/dig/PDF blobs (regenerable cache — §5.6). The T14 additive-cache test asserts the receiver *record* has `summaryHtml`/`digDeeperHtml` null/absent.
- **`baselineFromOneSided(classA: ClassASignals, mdHash: string | null, snapshot: HumanSnapshot): VideoBaseline`** — construct the manifest baseline for a fresh additive create (no reconcile): Class-A from `classA`+`mdHash`; Class-B from the present side's per-field `{ value, editedAt }`. After an additive create both replicas hold the present side's values, so this is a true agreed baseline.

**Servability of an additive PUBLISH (local→cloud) — round-3 L-R3-3:** a plain `upsertVideo` of a local record (which has no `artifacts` field) leaves the cloud row without `artifacts.summaryMd.status = 'promoted'`, so `summaryReady` stays false and the published summary isn't servable until an owner action. For M2a, `copyAdditiveVideo` publishing to the **cloud** must set `artifacts.summaryMd = { key: video.summaryMd, status: 'promoted' }` on the sanitized record (the copied MD is known-good — the blob was just written), so the published summary is immediately servable. For cloud→local hydrate there is no artifact-status concept (local serves off the MD file directly). Verify against the `summaryReady` derivation (`artifacts.summaryMd.status === 'promoted'`).
...
- **`buildBaseline(currentClassA: ClassASignals, currentMdHash: string | null, merges, previousBaseline: VideoBaseline | undefined): VideoBaseline`** — the manifest baseline written by `writeVideoBaseline` AFTER reconcile (Behavior #10). It records the **agreed post-reconcile state**, NOT a "winner" (round-2 N4). Class-A: for a `copyTo*` transfer pass the winner's signals + verified `mdHash`; for `skip` (both equal) the shared current signals + common `mdHash`. **Class-B — per field (round-3 H2, CRITICAL):** advance to `{ value: merges[f].value, editedAt: merges[f].editedAt }` ONLY when the field **resolved** (a write was applied, or both already agreed); for a **no-write conflict** (`merges[f].winner === 'equal' && merges[f].conflict`), carry **`previousBaseline?.classB?.[f]` UNCHANGED** (or `{ value: undefined, editedAt: undefined }` if none). Recording the winner's value as the baseline for a no-write conflict is a **false agreement**: next run the matching side looks unchanged and the other "changed", so the previously-skipped conflict silently **overwrites** the human's value — defeating §5.5. Preserving the prior baseline keeps both sides "changed vs baseline" → the conflict re-logs and stays non-destructive until the human resolves it. A baseline is advanced for **every reconciled two-sided video — including `skip`** — so §5.6 delete-inference has a 'seen-before' record next run. `mdHash` lives here (manifest), NOT on the `Video` record (Low finding).
...
export async function runSync(deps: SyncDeps, opts: { playlistKey?: string } = {}): Promise<SyncReport> {
  ...
  for (const key of keys) {
    ...
    await ensureHydrationRoot(dataRoot); // mkdir -p BEFORE any local read/write (round-5 H1 — fresh-device hydrate)
    ...
    for (const id of await enumerateVideoIds(deps.local, deps.cloud, localP, cloudP)) {
      try {
        ...
        // BEFORE any lv!/cv! dereference (round-2 N2 — a one-sided video must never reach the
        // two-sided reconcile, whose deriveHumanSnapshot(null) would NPE).
        if (!lv || !cv) {
          const present = (lv ?? cv)!;             // the side that has it
          const presentIsLocal = lv != null;
          if (base) {
            report.removed++;                      // in baseline + absent other side → deleted there; do NOT re-create (no propagation — M2b)
          } else {                                 // no baseline + present one side → additive create (money-safe)
            // tuple carries BOTH the receiver metadata store (to) and blob store (toBlob) — round-3 H1
            const [fromP, fromBlob, to, toP, toBlob] = presentIsLocal
              ? [localP, deps.localBlob, deps.cloud, cloudP, deps.cloudBlob]
              : [cloudP, deps.cloudBlob, deps.local, localP, deps.localBlob];
            const body = await readMdBody(fromBlob, fromP, present);
            // playlistMeta for `key` from the union entry (local registry playlistUrl, or cloud
            // listPlaylists PlaylistSummary) — needed so ensureReceiverSlot can create the playlist row.
            const playlistMeta = playlistMetaFor(key, localPlaylists, cloudSummaries);
            await copyAdditiveVideo(to, toP, toBlob, playlistMeta, present, body); // verifies receiver row (step 4) or throws
            report.created++; // reached only after the receiver row is confirmed to exist
            await writeVideoBaseline(dataRoot, key, id, baselineFromOneSided(
              deriveClassASignals(present, body), body ? mdHash(body) : null, deriveHumanSnapshot(present)));
          }
          continue;
        }

        // Both present — reconcile (Behavior #2 applies to TWO-SIDED videos only).
        // 1) Class B FIRST
        const merges = reconcileHuman(deriveHumanSnapshot(lv), deriveHumanSnapshot(cv), base?.classB ?? EMPTY_CLASSB);
        report.mergedFields += await applyClassBWinners(/* write each winner to its loser side, source ts */);
        // If `corrections` is a no-write conflict (both backfilled), this uses one side arbitrarily; the
        // only consequence is a possible needless needsRegen/unify (Class-A variants are equivalent),
        // never data loss — the corrections field itself is preserved by the Class-B conflict-skip.
        // Accepted residual (round-3 H2 secondary).
        ...
        if (decision.action === 'copyToCloud') { winnerMdHash = (await transferClassA(/* winner=local→cloud */)).mdHash; winnerSignals = la; report.updatedCloud++; }
        else if (decision.action === 'copyToLocal') { winnerMdHash = (await transferClassA(/* winner=cloud→local */)).mdHash; winnerSignals = ca; report.updatedLocal++; }
        else { report.skippedIdentical++; winnerMdHash = la.mdHash; winnerSignals = la; } // skip: sides already agree

        if (decision.action !== 'skip' && winnerMdHash) {
          if ((await companionTransfer(/* winner→loser */)).shareNeedsOwnerServe) report.shareNeedsOwnerServe++;
        }
        if (lv.archived !== cv.archived) report.archivedNotSynced++;

        // 3) Manifest AFTER verified commit (Behavior #10) — advances even on skip (N4); pass the
        //    PREVIOUS baseline (base) so no-write Class-B conflicts are NOT falsely advanced (round-3 H2).
...
| 12 | Backfilled Class-B conflict preserved across TWO runs | legacy divergent `personalNote` (both backfilled); run sync **twice** | both runs log+skip; NEITHER side overwritten; baseline not falsely advanced (round-3 H2, §5.5) |
| 13 | Additive create of a summary-less video | one-sided video with `summaryMd = null` | metadata copied; no blob `put`; no throw (round-3 M-R3-1) |
| 14 | Additive PUBLISH is servable | local→cloud additive create of a promoted summary | cloud row has `artifacts.summaryMd.status = 'promoted'` → `summaryReady` true (round-3 L-R3-3) |
| 15 | Additive publish CREATES the receiver row (not silent no-op) | local-only playlist+video published to a cloud with no such playlist | after sync, `cloud.readIndex` contains the playlist + video (ensureReceiverSlot ran); baseline written only after that; re-run does NOT read it as a delete (round-4 H1) |
| 16 | Promoted status never precedes a durable blob | additive publish where the blob write fails mid-way | no cloud row advertises `status='promoted'` without a readable MD blob; baseline not advanced (round-4 M1) |
| 17 | Fresh-device hydrate creates the local root | cloud-only playlist; `<dataRoots[0]>/<key>` does **NOT** exist on disk | sync `mkdir -p`s the root, writes `playlist-index.json` + the video + MD; re-run is not a delete (round-5 H1). **The test MUST point `dataRoots[0]` at an empty tmp dir with no `<key>` subdir — never pre-create it, or the mkdir path goes untested.** |
...
## Round-2 review dispositions (2026-07-17)

Round-2 dual re-review (Codex + Claude, verify-fixes + hunt-fix-introduced): both **NOT CONVERGED** — 0 Blocking, but the round-1 fixes introduced new defects (expected for a large rework). Round-1 Blockings all verified genuinely closed. Reviews: `docs/reviews/plan-cloud-sync-m2a-{codex,claude}-r2.md`. Addressed in this revision:

- **High:** N1 — `sourceMdHash` fix hashed the filename (`sourceMd`) not the body → use `mdHash(md)` (`generate.ts:33`), test asserts against BODY (T4c). H1 — Class-B value-equality returned `'equal'` and skipped the write → converge to newer-timestamp winner, `conflict:false` (T6). H2 — token `write()` chmod'd the parent before checking → check existing parent first, only self-heal a created dir (T10). H3 — additive create upserted cache pointers → `sanitizeAdditiveVideo` clears `summaryHtml`/`digDeeperHtml`/dig+PDF artifacts (T12). N2 — one-sided presence branch was `// ...` before `lv!/cv!` (NPE) → explicit `if (!lv||!cv){…continue}` before the two-sided reconcile (T12).
- **Medium:** N3 — local `updateVideoAnnotations` allowlist widened to include `corrections` + test (T4d). N4 — `buildBaseline` takes the agreed post-reconcile signals and advances even on `skip` (T12). N5 — bare regenerate stamps `mdHash(effectiveCorrections)` (stored value when the param is absent), not `mdHash('')` (T4b).
- **Low (accepted/documented):** N6 — `persist_summary` grant under `authenticated` is flagged as a must-verify-early gate in T12's RLS note (accepted). N7 — equal-`mdHash` + one-current + different-major downgrade is a degenerate/unreachable state (noted). N8 — `merge_video_data_bulk` is intentionally NOT given the conditional restamp (a membership/non-Class-B path sync never uses); documented as an intentional exclusion.

## Round-3 review dispositions (2026-07-17)

Round-3 dual re-review: both **NOT CONVERGED**, 0 Blocking; all round-2 Blocking/High verified genuinely closed. The round-2 fixes introduced localized new defects (fewer + smaller each round — the loop is converging). Reviews: `docs/reviews/plan-cloud-sync-m2a-{codex,claude}-r3.md`. Addressed here:

- **High:** H2/H-R3-1 (**both reviewers**, the critical one) — the round-2 `buildBaseline` fix advanced the manifest with a *false agreement* for a no-write (backfilled) Class-B conflict → next-run **silent overwrite** of a human field, defeating §5.5. Fixed: `buildBaseline` takes `previousBaseline` and **preserves the prior baseline for any no-write conflict field** (`winner:'equal' && conflict`), so the conflict re-logs non-destructively until the human resolves it; the `runSync` call passes `base`; added T14 row 12 (two-run regression). H1 (Codex) — the additive tuple omitted the receiver **blob** store → `copyAdditiveVideo(to, toP, toBlob, …)` now takes it explicitly; tuple threads `toBlob`.
- **Medium:** M-R3-1 (Claude) — `copyAdditiveVideo` guards the blob `put` when `summaryMd`/`mdBody` is null (summary-less one-sided video); T14 row 13. M1/L-R3-1 (**both**) — the stale T4 file-list header `mdHash(sourceMd)` corrected to `mdHash(md)` (body).
- **Low:** L-R3-2 (Claude) — whitespace-only `corrections` param now falls back to the stored value in `effectiveCorrections`. L-R3-3 (Claude) — additive **publish** (local→cloud) sets `artifacts.summaryMd.status='promoted'` so the published summary is servable; T14 row 14. H2 secondary (corrections no-write conflict → Class-A currency judged against one side) documented as a benign accepted residual (at most a needless `needsRegen`/unify; corrections field preserved by the conflict-skip).

## Round-4 review dispositions (2026-07-17)

Round-4: **reviewers split** — Claude **CONVERGED** (0 B/H/M, verified the two-run `buildBaseline` fix genuinely prevents the run-2 overwrite incl. the first-sync undefined-baseline case), Codex **NOT CONVERGED** (1 High + 1 Medium). **Adjudicated by reading the real source** (`supabase-metadata-store.ts:104-113`): Codex is correct — cloud `upsertVideo` is a bare `UPDATE` of a `claimVideoSlot`-created row, so an additive publish of a not-yet-existing cloud playlist/video silently no-ops. All round-3 findings verified genuinely closed by both. Addressed:

- **High (Codex H1):** additive publish never created the receiver cloud row → silent no-op → orphaned blob + false baseline → next-run mis-read as delete. Fixed: **`ensureReceiverSlot`** (idempotent `setPlaylistMeta` + guarded `claimVideoSlot`) creates the receiver playlist+video row first; `copyAdditiveVideo` **verifies `readIndex` contains the video before the baseline is written**; receiver-claimed `position`/`serialNumber` used (replica-local §4.1). T14 rows 15/16.
- **Medium (Codex M1):** promoted-status could be advertised before the MD blob was durable. Fixed: blob written+verified BEFORE setting `artifacts.summaryMd.status='promoted'`; ordering made explicit in `copyAdditiveVideo`.
- **Low (Claude nits):** L4 — promoted-status set is direction-agnostic (inert locally, required on cloud). L5 — `sanitizeAdditiveVideo` explicitly keeps `annotationsEditedAt` (avoids next-sync convergence churn) and drops the sender's replica-local ordering.

Convergence trend (Blocking 4→0→0→0; High 3→5→2→1) continues; the last High cluster was confined to the additive-create path, now hardened by reusing ensure-slot + verify + blob-before-promoted.

## Round-5 review dispositions (2026-07-17)

Round-5: **both reviewers AGREE** — NOT CONVERGED, 0 Blocking, **the same single High** (independently traced to the same source lines) + 1 Low. The round-4 publish-direction fixes verified genuinely closed by both. The remaining High is the **mirror (hydrate) direction**:

- **High (both):** a fresh-device cloud→local hydrate throws because nothing creates the local root directory before local `readIndex`/`setPlaylistMeta` touch it (`index-store.ts:70-78,83-96`) — breaking the spec §7 fresh-device pull. Fixed: **`ensureHydrationRoot(dataRoot)`** (`mkdir -p`) called at the top of the per-playlist loop, before `readManifest`/`enumerateVideoIds`. T14 row 17 requires a genuinely non-existent root so the mkdir path is actually exercised (both reviewers warned a pre-created harness dir would mask it).
- **Low:** `claimVideoSlot` wording corrected (`on conflict do nothing`, guard still correct — Codex L1); `ensureReceiverSlot`-returns-null fallback documented (keep receiver's existing ordering — Claude L1).

Both reviewers verified clean: presence-branch tuple direction, `playlistMetaFor` both directions, `baselineFromOneSided`, counter semantics, money path, `readManifest` degrade-on-missing-root. This was a concrete agreed one-line fix, not a design fork — no human decision required.
