...
1,541 lines of duplicated source are replaced with pointers.
...
```python
    ...
    with tempfile.TemporaryDirectory() as _td:
        ...
        _mut = [{"name": "f returns two", "file": "m.py",
                 # ANCHORED on the def line: bare "return 1" occurs TWICE in this
                 # fixture (f's body and _self_test's failure branch), and the engine
                 # REFUSES an ambiguous anchor — so the case would have failed for a
                 # reason unrelated to what it tests. Found in review round 1.
                 "edits": [["def f():\n    return 1", "def f():\n    return 2"]],
...
# ANCHORED to line starts, deliberately. A raw `"<!-- file:" not in s` test FAILS on
# prose that must survive: :3055 documents the `<!-- file: ../escape.py -->` path-escape
# defect inside a table cell, and that record is worth keeping. Found in review round 1.
assert not re.search(r"^\s*<!--\s*file:", s, re.M), "a real file BLOCK remains — refusing"
assert not re.search(r"^\s*<!--\s*mutations\s*-->\s*$", s, re.M), "mutations block remains"
p.write_text(s)
...
gone and its 1,541 duplicated lines with it. ⚠ Coupling REDUCED, not eliminated: 1,551
...
The plan's 1,541 lines of duplicated source are DELETED, not banner-warned. Once
