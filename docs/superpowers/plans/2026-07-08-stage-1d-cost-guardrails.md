# Stage 1D — Cost Guardrails Implementation Plan (v2)
...
## v2 — review fixes applied (round-1 dual plan review: `docs/reviews/plan-stage-1d-{codex,claude}.md`)
...
beforeEach(async () => {
  ...
  await svc.from('guardrail_config').update({ daily_cap_cents: 500, summary_est_cents: 150, summary_max_attempts: 1,
    max_duration_seconds: 1800, velocity_per_ip_hourly: 15, max_queue_depth: 200, max_free_users: 100, captcha_soft_threshold: 5 }).eq('id', true);
...
- [ ] **Step 1: Failing tests** (reuse `beforeEach`): set `velocity_per_ip_hourly=2`; enqueue 3 from `ip='9.9.9.9'` (pass `ip` to `enq`); `enqueue_preflight('9.9.9.9', ownerId)` → `velocity_exceeded:true`, and the row has exactly keys `admitted/at_capacity/velocity_exceeded/challenge_required`. A **different** IP with 0 recent jobs → `velocity_exceeded:false`. A client-session `rpc('enqueue_preflight',...)` → `error` (execute revoked).
- [ ] **Step 2: Run — FAIL.**
- [ ] **Step 3: Implement** per spec §5: `security invoker`, guard `auth.role()='service_role'`, read `guardrail_config`; `velocity_exceeded = (select count(*) from jobs where enqueue_ip=p_ip and created_at > now()-interval '1 hour') >= velocity_per_ip_hourly` (uses `jobs_velocity`); `at_capacity = today reserved+actual >= daily_cap_cents OR (count queued+active) >= max_queue_depth`; `admitted = NOT is_anonymous(p_owner_id) OR (anon rank by profiles.created_at <= max_free_users)`; `challenge_required = is_anonymous AND per-IP hour count > captcha_soft_threshold`. Booleans only.
...
- [ ] **Step 2: FAIL. Step 3: Implement** `gemini-cost.ts`; refactor `gemini.ts` to import the retry constants + export the resolved models.
...
**Files:** Modify `types/index.ts` (`VideoMetaSchema` gains `liveBroadcastContent: z.string()` — YouTube returns `'none'|'live'|'upcoming'`), `lib/youtube.ts` (`fetchPlaylistVideos` maps `snippet.liveBroadcastContent`), `lib/job-queue/video-meta-to-payload.ts` (unaffected — the field is used by the producer, not the payload); Test `tests/lib/youtube.test.ts`.

- [ ] **Step 1: Failing test** — `fetchPlaylistVideos` fixture with `snippet.liveBroadcastContent:'live'` surfaces `liveBroadcastContent:'live'` on the `VideoMeta`; a normal video → `'none'`.
- [ ] **Step 2: FAIL. Step 3: Implement** the schema field + youtube mapping. **Step 4: PASS + `npx tsc --noEmit`. Step 5: Commit** (`feat(1d): VideoMeta.liveBroadcastContent for VOD-only`).

---

### Task 10: Producer two-client split + buckets + VOD/too_long block; remove `SupabaseJobQueue.enqueue`

**Files:** Modify `lib/job-queue/producer.ts`, `lib/storage/supabase/supabase-job-queue.ts`, `lib/storage/job-queue.ts`; Test `tests/lib/producer-guardrails.test.ts`.
...
- [ ] **Step 1: Failing tests** (fake `Enqueuer` with `getGuardrailConfig` → `{maxDurationSeconds:1800}`): over-duration video **and** `liveBroadcastContent!=='none'` → `too_long`, never passed to `enqueue`; quota exhausts mid-list → per-video `quota_exceeded`, rest still enqueue; `DailyCapError` mid-loop → that + remaining `daily_cap`, `dailyCapReached:true`; enqueue receives `{ownerId, enqueueIp}`; **disjoint sum** `enqueued+joined+skipped+failed+quotaBlocked+capBlocked+tooLong === videos.length`. Add a case where the PJ003 backstop fires **inside** the loop (a video whose duration passes the producer check but the RPC throws `VideoTooLongError`) → counted in `tooLong`, and assert the sum still holds.
- [ ] **Step 2: FAIL. Step 3: Implement** — `maxDurationSeconds = (await enqueuer.getGuardrailConfig()).maxDurationSeconds`; block over-duration + `liveBroadcastContent!=='none'` before enqueue (`tooLongPreBlock`); fan out via `enqueuer.enqueue`, catch `QuotaExceededError→quotaBlocked/continue`, `DailyCapError→capBlocked + dailyCapReached + cap-block remaining`, `VideoTooLongError→tooLongInLoop`; **`failed = enqueueable.length - created - joined - quotaBlocked - capBlocked - tooLongInLoop`** (Claude M1); `counts.tooLong = tooLongPreBlock + tooLongInLoop`. Delete `SupabaseJobQueue.enqueue` + the `enqueue` member of `JobQueue`.
...
- [ ] **Step 2: Inventory** — run `grep -rln "enqueue_job\|\.enqueue(" tests/integration/` and migrate each: `job-queue-schema.test.ts` (with-check → grant 42501; idempotency → `enqueue_job` join, no `.error`), `cancel-by-playlist`, `cancel-job-rpc`, `job-queue-runner`, `job-queue-store`, `job-queue-producer`, `job-queue-playlist-identity`, `job-queue-worker`, `worker-main`, `jobs-producer-polling`, `producer-roundtrip` — switch direct session `enqueue_job`/`SupabaseJobQueue.enqueue` to the **8-arg service path** (`svc.rpc('enqueue_job',{p_owner_id,…,p_enqueue_ip})` or `SupabaseEnqueuer`); re-baseline `jobs-producer-polling`/`producer-roundtrip` counts against the two-client producer + new buckets. (Classify per file: direct-RPC vs producer-roundtrip vs admin-insert-stays-valid.)
