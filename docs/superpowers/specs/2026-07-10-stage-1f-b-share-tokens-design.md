...
| D3 | ⟳ **SUPERSEDED 2026-08-11 (M1.4 item B4) — now "serve-if-title-stable, else not ready."** A model whose `generatorVersion` differs is **SERVED** (`readTitleStableModel`); only an absent model, drifted titles, a schema-invalid envelope, or a model with fewer sections than the markdown → coarse not-ready. Never generate, either way. **Was:** *"Serve-if-fresh, else not ready. Model absent/stale (`isFresh` false) → coarse not-ready."* | The old rule's recovery — "the owner materializes by viewing once" — is **owner-dependent**, so a link to a doc its owner never revisits stayed broken after every version bump, and skew is the normal state during a rolling deploy. The owner path keeps `isFresh` (it can pay to heal); the share tolerates. Freshness is no longer a single source of truth across both paths, and that is the deliberate trade. |
...
| D13 | **Extract a generate-free leaf module `lib/html-doc/read-model.ts`** exporting `readFreshMagazineModel(...)` (does `readModelEnvelope` + `isFresh`, **no RPC, no generate**) and `isFresh`. *(2026-08-11: the leaf also exports `readTitleStableModel`, which the share route now uses — see B8. The generate-free property is unchanged.)* It imports **only** `readModelEnvelope` + `GENERATOR_VERSION` — **never** `@/lib/gemini`, `@/lib/gemini-cost`, `serve-doc`, or any `reserve_*`. To keep it a **true leaf**, extract `GENERATOR_VERSION` into a tiny `lib/html-doc/constants.ts` that both `render.ts` and `read-model.ts` import (so the freshness helper does not drag the whole renderer graph). `serve-doc.ts` imports the helper from `read-model.ts` and uses it at **both** its read sites; the share route imports **only** `read-model.ts`. | Puts the helper where its module graph is provably Gemini-free — so importing it into the anonymous route cannot pull in the charging code. This is the structural core of "never charges." Touching merged 1F-a code (`serve-doc.ts`) → re-review trigger (§8). |
...
4. **Read-only reads (D13/D16):** wrap the service-role blob store as `{ get: store.get.bind(store) }`. Read the MD blob; **if the MD blob is missing behind a `promoted` record ⇒ coarse 404** (the anon analogue of the owner path's `repair needed`, `route.ts:60` — no repair hint leaked). Then `parseSummaryMarkdown` (wrapped: a parse throw on corrupt MD is caught ⇒ coarse **404**, **never a 500 leak**) → titles → `base = mdKey.replace(/\.md$/,'')`. Call **`readTitleStableModel({ blobStore: readOnly, principal, base, titles })`** from `read-model.ts` — no RPC, no generate. *(Was `readFreshMagazineModel` until 2026-08-11; the share now TOLERATES generator-version skew — see B8.)* `none` (absent model, drifted titles, schema-invalid envelope, or a model with fewer sections than the markdown) ⇒ **"not ready"** (503-class). The share module imports none of `resolveMagazineModel` / `generateMagazineModel` (and calls no `reserve_serve_model` RPC).
...
| B8 | Serve when model is version-skewed (version bump) | live token, `sameTitles` true, `generatorVersion` differs | ⟳ **REVERSED 2026-08-11 — 200, serve the skewed model** (M1.4 item B4, product decision: *tolerate*). Was: "not ready"; heals after owner next views |
| B8b | Serve when model titles drifted | live token, `sameTitles` false | **"not ready"** (503-class); a drifted model would mis-pair positionally |
| B8c | Serve when model SHAPE is invalid under the current schema | envelope fails `ModelEnvelopeSchema.safeParse` | **"not ready"** (503-class); zod is the structural gate |
...
- **Unit** — token gen/hash helper (shape, 32-byte hash, base64url); TTL route contract (omitted→30, `'never'`→null, bounds→400); `readTitleStableModel` (ok when titles match — version ignored; none when drifted or absent; spies assert **zero** reserve/generate calls); share-mode render strips the enumerated set (B22 fixtures with `source-md`/`video-id`/`generator` present).
