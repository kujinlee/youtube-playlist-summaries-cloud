# Reservation Release Lifecycle — Design Spec (v5)

**Date:** 2026-07-15 (v3–v5: 2026-07-16)
**Status:** Draft v5 (revised after round-4 dual adversarial review — see §15). Pending re-review (round 5) + user approval.
...
|---|---|---|---|
| **A — Pre-send / positively-rejected** | bad input / payload validation; duration-cap; magazine caps-missing / input-cap `NonRetryableError` (`gemini.ts:60,85,505`); transcription fail-closed `NonRetryableError` (`gemini.ts:658`); **a Google API error carrying HTTP status ∈ {429, 503}** (rate-limited / overloaded → refused *before* generation → 0 tokens → $0) — **and only when no earlier attempt may have metered** (see retry rule below) | **no (provable)** | **RELEASE** |
...
**The classifier (single testable helper, `classifyGeminiFailure(err, ourSignal)` → `'release' | 'keep'`).** Walks the `.cause` chain `gemini.ts` populates (`new Error(msg,{cause})` at `:394,:441,:554`):
1. **our lease-abort? → `'keep'`.** `if (ourSignal?.aborted) return 'keep'`. Keying on `ourSignal.aborted` is **necessary, not stylistic** — an SDK-internal abort surfaces as `GoogleGenerativeAIAbortError` with `.name === 'Error'` (not `'AbortError'`), so `err.name`/`instanceof` cannot distinguish our lease-abort (requeue, never release) from an SDK timeout (class B KEEP) — only our own signal can (CL4-L1).
2. **any attempt may have metered? → `'keep'`.** `if (err.maybeMetered === true) return 'keep'` — the retry-aggregate flag (below). This gates C4-B1: a prior metered attempt then a final 429/503 must NOT release.
3. **class-A pre-send / rejection → `'release'`.** `err` (or any `.cause`) `instanceof NonRetryableError` → `'release'`; a `GoogleGenerativeAIFetchError` (or our dig `GeminiHttpError`) whose `.status ∈ {429, 503}` → `'release'`.
4. **everything else → `'keep'`** (timeouts, non-lease abort, 400/500/502/504, SDK-stripped connection errors, post-return validation).

The runner passes `p_billable_succeeded = (classify(err, ourSignal) !== 'release')` (§5 M2). No token counts, no cost math.

**Three lib changes make the class-A signal actually reachable (round-4 — the classifier is only as good as the errors the code emits):**
- **Retry-aware `maybeMetered` (C4-B1).** The retry loops — `generateJson` (`gemini.ts:252-269`), `transcribeViaGemini` (`gemini.ts:665-689`), and dig's retry (`generate.ts:256-266`) — must set `maybeMetered = true` on the finally-thrown error if **any** attempt received a response body (billed) *or* was itself class B (timeout/ambiguous). Then a "metered attempt 1 → parse fail → 503 on attempt 2" aggregates to `maybeMetered=true` → step 2 → KEEP. Without this, the classifier sees only the final 503 and wrongly RELEASEs a metered generation (under-count).
- **Dig throws a typed status-bearing error (C4-H1).** `generateDig` (`lib/dig/generate.ts:268-271`) currently throws a **generic** `Error("...HTTP ${res.status}")` with no `.status` — so a dig outage is invisible to the classifier and dig KEEPs (its 150¢ never releases; the §1 closure is false for dig). Fix: throw a typed `GeminiHttpError { status }` (or attach `.status`) that the classifier recognizes, and include dig in the `maybeMetered` retry logic. (Dig uses a hand-rolled REST helper, not the `@google/generative-ai` SDK — so it needs its own typed error, distinct from `GoogleGenerativeAIFetchError`.)
- **Transcript wrapper preserves the TYPED error, not the first non-null (CL4-H1).** `resolveTranscriptSegments` (`transcript-source.ts:62`) wraps `{ cause: captionErr ?? geminiErr }`. Because `fetchTranscriptSegments` **throws** on no-captions (`youtube.ts:90`), `captionErr` is truthy for **every** caption-less cloud video, so the typed class-A `NonRetryableError` (fail-closed transcribe, `gemini.ts:658`) is discarded → classifier sees a generic error → KEEP (a $0 leak). Fix: when the Gemini fallback threw a typed error, re-throw it (or set `{ cause: geminiErr }`) so `NonRetryableError` survives to the classifier.
...
| Bad input / duration-cap / caps `NonRetryableError` (pre-send, typed) | A | **RELEASE** |
| Gemini API status ∈ {429, 503}, no prior metered attempt (summary/magazine SDK **or** typed dig `GeminiHttpError`) | A | **RELEASE** |
| Gemini **timeout** / non-lease `AbortError` / 500 / 502 / 504 / SDK-stripped connection error | B | **KEEP** |
| A prior retry attempt metered, then a final 429/503 (`maybeMetered=true`) | B | **KEEP** |
...
The runner classifies the caught error via the single helper `classifyGeminiFailure(err, ourSignal)` (§3.1) and passes `p_billable_succeeded = (classify(err, ourSignal) !== 'release')`. So `false` (RELEASE) fires only for §3.1 class A: a pre-send `NonRetryableError` (caps/input-cap/transcription-disabled/duration-cap) or a Gemini rejection status ∈ {429, 503} with no prior metered attempt. Class B (timeouts, non-lease `AbortError`, 500/502/504, SDK-stripped connection errors, `maybeMetered=true`) and class C (post-return) → `true` (KEEP).

**Implementability (§3.1 lib changes) — the discriminator must survive to the runner.** Round-3/round-4 found the class-A signal is currently unreachable on three paths. §3.1 pins the fixes; in short: (1) the retry loops set `maybeMetered` so a prior-metered attempt then a 429/503 KEEPs (C4-B1); (2) `generateDig` throws a typed `GeminiHttpError { status }` so dig outages are visible (C4-H1); (3) `resolveTranscriptSegments` preserves the **typed** Gemini error (not `captionErr ?? geminiErr`, which drops it because the caption fetch always throws — CL4-H1). `classifyGeminiFailure` then walks the `.cause` chain `gemini.ts` already populates. The connection-`.code` branch is **dropped** — the SDK's `handleResponseError` strips `.code` (CL4-H2), so connection-only errors KEEP (a small documented residual; the dominant outage returns a 429/503 *status*, which is detectable).
...
- **Throw** → classify with the **same** `classifyGeminiFailure(err, ourSignal)` helper (§3.1). `'release'` (class A — pre-send `NonRetryableError` at `gemini.ts:505`/`:85`, or a Google API rejection with status ∈ {429, 503} and no prior metered attempt) → `settle_serve_model(token, released:=true)` (refund the 6¢). `'keep'` (class B/C — timeout, non-lease `AbortError`, 500/502/504, SDK-stripped connection error, `maybeMetered`, section-count/parse mismatch at `gemini.ts:547`, or the write step threw) → `settle_serve_model(token, released:=false)` (KEEP the charge; server may have metered) — this is the fix for the round-3 serve under-count. Then re-throw either way.
...
| 2b | Class-A Gemini REJECTION {429,503} releases (B-2) | `generateSummary`/`generateMagazineModel` throws `GoogleGenerativeAIFetchError.status ∈ {429,503}`, no prior metered attempt; `classify='release'`; `fail_job(billable=false)` | **released** — the §1 outage case; budget re-opens |
...
| 3e | Prior-metered retry then 429/503 KEEPS (C4-B1) | attempt 1 gets a response then parse-fails → retry → attempt 2 returns 503; aggregate `maybeMetered=true`; `classify='keep'` | **no** release (KEEP — real spend on attempt 1 not refunded) |
...
| 26 | Outage self-DoS is CLOSED for the HTTP-status storm (B-2) | N generations (summary/magazine/dig) all hit Google **503** (class A, no prior metered attempt) → all release | after N releases the ledger is back to baseline; a fresh `enqueue_job` admits — the dominant §1 outage no longer self-DoSes |
...
- **Gemini failure classification (§3.1, B-2 / round-4):** RELEASE only a positively not-metered class-A failure (pre-send `NonRetryableError`, or Google API status ∈ **{429, 503}** with no prior metered attempt); KEEP class B (timeout/non-lease-abort/500/502/504/SDK-stripped-connection/`maybeMetered`) and class C (post-return). Applies to **both** generation and serve, and to the typed-dig path. Unrecognized → KEEP (safe). Requires the retry-aggregate `maybeMetered`, the typed dig `GeminiHttpError`, and the transcript wrapper preserving the typed cause; plus one-time live verification (§9).
...
- `GoogleGenerativeAIFetchError` with `.status ∈ {429,503}`, no prior metered attempt → `release`; with `.status ∈ {500,502,504}` → `keep` (CL4-M1).
...
- an error with `maybeMetered=true` (aggregate from a prior-metered retry) even if the final status is 503 → `keep` (C4-B1).
...
Money path + concurrency + idempotency → **iterative dual adversarial review to convergence** (Codex + Claude, independent), re-reviewing the revised SQL each round until a round returns no new Blocking/High. Round-1 → v2, round-2 → v3, round-3 → v4, round-4 → v5 (all NOT CONVERGED; docs in `docs/reviews/reservation-release-spec-v{1,2,3,4}-*`). **Round-5 explicit targets** (verify the round-4 classifier fixes are genuine + hunt defects they introduced): the `maybeMetered` retry-aggregate genuinely covering every metered-then-rejected path in `generateJson`/`transcribeViaGemini`/`generateDig` (C4-B1 — no under-count survives); the typed dig `GeminiHttpError` reaching the classifier and its retry participating in `maybeMetered` (C4-H1); the transcript wrapper preserving the typed `NonRetryableError` when the caption fetch also threw (CL4-H1); the narrowed {429,503} release set + dropped connection branch being correct and the §9 tests asserting only code-real shapes (CL4-H2/M1); and whether the classifier is now **complete** (no class-A mis-KEPT that re-opens a self-DoS, and critically no class-B/C mis-RELEASED under-count). Also: the SQL bodies (cancel RPCs, `fail_job`) remain correct — they were verified closed in round-4 and are unchanged in v5.
