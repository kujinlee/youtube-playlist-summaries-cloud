...
> **dig-state is filename-authoritative (whole-branch review v1, M1 — accepted by design).** dig-state reports a section as dug based on the *presence* of a current-version blob filename; it does **not** open or parse the blob. The serve loader (Unit A) does open each blob and skips any that are malformed or vanished-after-list (behavior 19). Consequence: for a **corrupt/foreign current-version blob**, dig-state may report a section as dug while the dig-deeper serve omits it (and 404s if it was the only one). This is the intended "cheap, derived state query" contract — dig-state is an index, not a render validation — and it costs zero blob reads per poll. In practice the two agree (the generation backend writes valid blobs consistently); they diverge only under blob corruption, which has no money/isolation impact. If the future frontend slice needs exact render-parity, it can validate against the serve response rather than pushing per-section parse cost into this polling endpoint.
