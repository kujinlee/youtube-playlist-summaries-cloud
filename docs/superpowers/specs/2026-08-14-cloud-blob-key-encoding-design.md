...
`docs/reviews/spec-blob-key-encoding-r{1..9,11..18}-{codex,claude}.md`,
...
> ## ✅ ROUND 16 RAN. THE FALSIFIER DID NOT FIRE — and the two halves SPLIT on that question
>
> **Both dominating points were attacked head-on by independent enumeration and both HELD.** `mechanism`
> findings this round: **zero**.
>
> **The halves disagreed, which is the reason this project runs two.** Both found that `serialize` is
> bypassed by `reconcileCloudBase`'s byte-copy. The **Codex** half graded it **Blocking** and declared
> the falsifier fired. The **Claude** half found the *same* bypass **plus a second one Codex missed**
> (`serial-migrate-exec.ts:141`, a direct `fs.writeFileSync`) and graded both **Low**.
>
> **Adjudicated in favour of Low, by reading the code rather than counting verdicts.** Both bypasses are
> *transforms of an already-conforming envelope* and both preserve unknown fields, so neither can
> produce an envelope lacking `videoId` that did not already lack one: a relocated legacy envelope
> **propagates** a legacy state, it does not **introduce** one, and Codex's own harm scenario is
> unchanged from before the relocation. **A copy has no author to demand `videoId` from.** What survives
> is real and is fixed as §3.6.4's L2 note: the *sentence* was an over-broad universal — this document's
> **fourth** — and the mutation set had a genuine hole, now behavior **18j7**.
>
> **The round's one Blocking is the OPPOSITE shape: over-reach, not under-reach.** The adopt guard is
> direction-agnostic, and `copyAdditiveVideo` runs in both directions — see §3.5.1 placement 3. Refusing
> on **cloud→local** would strand a paid artifact whose every other route is already 409 or closed by
> this same version. It is `branch-coverage`, not `mechanism`: the two directions belong to `runSync`,
> not to this design, so no redesign can delete that branch — only fail to mention it.
>
> **The falsifier for round 17, in the same spirit:** fires to REDESIGN if round 17 produces another
> finding of the form *"a placement is stated for one branch/direction of the path it sits on"*. That
> would be the **third** — **26c3** was the first and **B1** the second, one row apart in the same table
> — and it would mean placements are still being chosen against function *names* rather than against the
> *branch set* §3.6.1b demands.

> ## ⛔ ROUND 17: THE FALSIFIER FIRED. The REDESIGN is of the INSTRUMENT, not the design
>
> **The halves split again, opposite to round 16.** Codex returned **CONVERGED** (1 Low) and verified
> §3.5.1b row by row. Claude returned **NOT CONVERGED** with a **Blocking**, and it was right —
> confirmed at `reconcile-serial.ts:150-155` **before** the finding was read.
>
> **The third instance was in a row the table called "One branch" and that Codex independently verified
> as one branch.** Both were correct *about the direction*: `cloud: cloudSide` really is hard-wired. The
> **value** `newBase` has two producers, one ternary apart, and the design was written for one of them.
>
> **What the escalation buys, stated as a decision rather than a dodge:** `mechanism` findings this
> round: **zero**, for the second round running. By the repaired discriminator — *can a redesign remove
> it?* — a redesign of §3.5/§3.6 removes nothing here; the credential, the seam and the two dominating
> points all held under attack for the third consecutive round. **What failed is §3.5.1b**, which was
> built to pre-empt exactly this and asked a question adjacent to the defect class. So **§3.5.1b is
> rebuilt against the guarded value** (above), and the design is not redesigned.
>
> This is the **second** escalation in this slice narrowed rather than honoured literally, and that is
> recorded here rather than left implicit, because a rule overridden twice without comment has been
> retired in practice. **The user was notified and can reverse it.**
>
> **The falsifier for round 18** — and note it now targets the rebuilt instrument, not the design:
> fires to REDESIGN if round 18 finds **a fourth instance under the OPERAND question**, i.e. a guarded
> value with a producer §3.5.1b's rewritten table does not name. If the re-asked question holds, the
> design has earned its exit from Phase 1; §3.1–§3.4 and §3.6 have been stable for several rounds and
> neither half found anything new in them this round.

> ## ⛔ ROUND 18: THE FALSIFIER FIRED — and this time it bought a SCRIPT, not a table
>
> **Both halves found the same missed producer** (`mdKey`'s `??` at `lib/share/serve.ts:47`, row 6),
> and the Claude half additionally found a **Blocking the Codex half missed**: v20's own B1 fix left
> the round-17 Blocking open through the seam, and added a duplicate set of paid blobs on the way.
>
> **The count that decides the response:**
>
> | round | missed producer | table version | severity |
> |---|---|---|---|
> | 15 | `transferClassA` runs in two directions | (no table yet) | Medium |
> | 16 | the adopt guard runs in two directions | (no table yet) | **Blocking** |
> | 17 | `newBase` comes from a **ternary** | v19's, asking about *direction* | **Blocking** |
> | 18 | `mdKey` comes from a **`??`** | v20's, asking the *right* question | Medium |
>
> v20's table asked the **correct** question and was verified row-by-row by two reviewers, and a
> producer still walked through it — one row from a defect the coordinator had just written *"a `??` is
> two producers"* about, in the brief for that very round.
>
> **So the honest conclusion is the one the falsifier promised: hand-enumeration is the wrong
> instrument, and what is owed is a script.** Noticing the *absence* of a branch, on a line you have
> already read and understood, is not a task humans or models do reliably — but it has a mechanically
> detectable signature, and **both** measured misses were a branch operator in the cited defining
> expression.
>
> **`scripts/check-producer-enumeration.py`** now checks this table on every run. Writing it
> immediately surfaced three defects nobody had filed: row 5's citation pointed at a signature
> continuation line, row 6's at a function instead of an assignment, and row 3's natural citation
> (`newBase = d.to!`) is a **bare alias** whose branch lives two lines inside another function — a
> false negative the script now refuses rather than passes.
>
> **The design itself was not the problem in either round.** Every mechanism check either half ran
> this round passed: the `oldBase` conjunct does not reopen round-14 B1, 26d3 is constructible, 26f's
> rewritten observable discriminates, the 41 call sites held under recount, 18j8 is writable, and L5's
> enumeration is complete.
>
> **Round 19's falsifier, and it is now aimed at something a script can settle:** fires to REDESIGN if
> round 19 finds a missed producer **that `check-producer-enumeration.py` reports as PASSING** — i.e.
> a defect in the instrument's coverage rather than in someone's attention. A miss in one of the three
> `UNCHECKABLE-BY-DESIGN` parameter rows is **not** that: those are declared, not claimed.
...
> enumerated class; **the metadata seam plus its three stated placements** (§3.5.1) exist so that class
> is refused *before* anything is durable or paid. *(Round-16 L1: this sentence still said "the mint and
> adopt call sites" — the v10 design. Round-15 L2 fixed the identical sentence in §7 and missed this
> one, which is what a stale cross-reference does: it survives the sweep that was aimed at its twin.)*
>
...
> - The adopt refusal (**location: §3.5.1b row 4** — this bullet has now named a stale site three times,
>   at rounds 13, 15 and 18, which is why it no longer names one at all) is per-video, caught, and
>   **advances no baseline**, so it **re-fires on every subsequent run, forever**, until a human renames
>   the vault file. That is the right behaviour under decision ③ — the automated repair was
...
> **Round-17 L5 — a FOURTH `MetadataStore` caller writes `summaryMd`, under a caption that says
> "verified by enumeration".** `lib/serial-migrate-exec.ts:130` builds `fieldUpdates[op.field] = op.to`
> where `op.field` can be `'summaryMd'` (`:131`), and calls `store.updateVideoFields` at `:146`.
> Harmless today, and for the reason the seam design makes structural rather than lucky: `runPhaseB`
> resolves its store through `getStorageBundle()` with **no client**, which throws under
> `STORAGE_BACKEND=supabase` (`resolve.ts:56`) — the same argument already made for
> `bulkUpdateVideoFields`. **Listed because a table captioned *verified by enumeration* must either be
> complete or stop saying that.**
>
...
> (`pipeline.ts:265` **and `:339`** — round-18 L3: this note named one write where there are two) is not a cloud advertisement and §3.4 argues at length that the local path never
...
**The placements outside the seam are the ones §3.5.1b enumerates — this text stops counting them**
*(round-17 L3: v19 said "THREE" and §3.5.1b lists four, the share guard being the one omitted; round-15 H2 was the identical defect one round earlier, when v17 said "two". A count restated beside a list is a second copy that drifts.)*, **each for a stated reason** *(round-15 H2: v17 said "two",
...
2. **`reconcileCloudBase` refuses IN MEMORY, before the copy phase**, alongside its existing
   ...
   *"the smallest correct behaviour: it cannot half-move anything"* — so **no blob is copied and
   ...
   > ### ⛔ ROUND-17 B1, Blocking — the refusal was written for ONE of `newBase`'s TWO producers
   >
   > ```ts
   > // reconcile-serial.ts:152-154 — describeDivergence. A TERNARY IS TWO PRODUCERS.
   ...
   > ```
   >
   > `reconcileCloudBase` takes `const newBase = d.to!` (`:186`), so the guard tests whichever arm
   > produced it. **The spec described arm A three times and never mentioned arm B** — *"the vault
   > filename, verbatim"*, behavior 26d's *"rename the file"* repair, and §3.5.1b row 3's *"One"*.
   >
   > **Arm B is reached when the local row has a serial but no MD**, a state this document already
   > asserts exists (§3.6.2's round-11 L3 note), produced by `pipeline.ts:235` (slot claimed before
   > `writeSummaryDoc`) and by `ensureReceiverSlot` on a local receiver. With a cloud row carrying an
   > unservable `summaryMd` — §3.5's own reachability table, and §4 performs **no migration**, so
   > existing ones persist — the refusal fires and:
   >
   > ```
   > :729  reconcileCloudBase  → refuse → :756 throw    ← the guard fires here
   > :771  reconcileClassA     → NEVER REACHED
   > :791  copyToLocal / transferClassA → NEVER REACHED   ← the hydration round-16 B1 exists to preserve
   > ```
   >
   > **It buys nothing on this arm and costs everything.** Placement 2's justification is that a
   > relocation *"copies every paid blob to the unservable base … and deletes the sources"* — it turns a
   > **servable** artifact unreachable. On arm B the old base was *already* unservable; the relocation
   > only renumbers it. Nothing servable is at risk, and the refusal's entire effect is to block the
   > paid summary's last remaining route to the user — **permanently**, since no baseline advances and
   > the state reproduces itself. That is round-16 B1's harm arriving through a different door.
   >
   > ### ⛔ ROUND-18 B1 — v20's fix DID NOT FIX IT, and made the failure more expensive
   >
   > v20 scoped the in-memory refusal so arm B proceeds. **But `reconcileCloudBase` writes its row
   > through `updateVideoFields` (`reconcile-serial.ts:324`) — which §3.5.1's own entrance table lists
   > as going through the SEAM**, and the seam refuses unservable keys with no exemption anywhere:
   >
   > ```
   > in-memory guard  → passes (v20's conjunct)
   > :281-290         → EVERY PAID BLOB COPIED to the new base
   > :324 the seam    → REFUSES the row write
   > → 'metadata-failed' → throw at :756 → no baseline
   > :771 reconcileClassA / :793 copyToLocal → STILL NEVER REACHED
   > ```
   >
   > Identical harm to round-17 B1 — the paid summary unreachable through every product path — **plus a
   > full duplicate set of paid blobs at a base nothing will ever advertise**, and an operator message
   > that no longer names the cause. **A regression against v19 on cost:** v19 refused in memory and
   > copied nothing.
   >
   > **And this document states the failure mode two lines from where v20 implemented it** — placement 2
   > exists *"so no blob is copied and nothing is deleted, rather than relying on the seam to reject
   > after the copy"*. **v20 made placement 2 NARROWER than the seam it front-runs, and a guard narrower
   > than the one it front-runs is not a front-run.**
   >
   > **Why `mechanism`:** one rule — *an unservable key must not become a cloud advertisement* — was
   > stated in two places with **different predicates** and no account of which wins. That is the
   > *two mechanisms for one concern* shape this project has already paid for once.
   >
   > **FIX, part 1 — the conjunct discriminates refuse-vs-SKIP, not refuse-vs-proceed.** The
   > relocation's purpose is to keep the cloud's derived-blob addresses in step with local's serial;
   > when the resulting key **cannot be advertised at all**, there is nothing to keep in step:
   >
   > | old base | new base | outcome |
   > |---|---|---|
   > | servable | servable | **relocate** — today's normal path |
   > | servable | unservable | **refuse** `unservable-base` — protect a working advertisement (26d3). `applySerial` can do this by widening the serial prefix past 131 code points |
   > | unservable | servable | **relocate** — a genuine repair, and the seam accepts it. **No behavior covered this cell before round 18** (26d4) |
   > | unservable | unservable | **SKIP** — `{ ok: true, action: 'skipped-unservable' }`. No copy, no seam write, and `reconcileClassA` → `copyToLocal` runs, which is exactly what round-17 B1 wants |
   >
   > **⚠ The fix gives this guard a SECOND operand (round-18 L2).** It now tests `oldBase` as well as
   > `newBase`, and §3.5.1b row 3 names only `newBase`. `oldBase = d.from!`, aliased from
   > `baseOf(cloudVideo.summaryMd)` (`reconcile-serial.ts:151`) — **one producer, no branch**, which is
   > why it is safe; but a guarded operand absent from the table is exactly what the table exists to
   > prevent. Row 3's value cell names both.
   >
   > **This never asks the seam to accept an unservable key, so §3.5.1b row 1 stays BLIND and stays
   > TRUE** — the design's strongest claim, and the one v20 quietly broke. The skip must be **visible,
   > not silent**: push the unrepaired divergence into `report.errors`, the shape
   > `sync-run.ts:713-716` already uses for the corrections-unresolved path. A new `ok: true` action is
   > a closed-union addition, mechanically identical to the refusal variant round-15 M3 validated.
   >
   > **FIX, part 2 — the message names the right subject per arm.** Carry the producer in the variant:
   > **`{ ok:false; reason:'unservable-base'; key: string; origin: 'vault-filename' | 'cloud-key' }`**
   > (**round-18 L1: derive `origin` from the SAME branch as `to`** — ideally by returning it from
   > `describeDivergence` alongside `to`. v20 specified `localVideo.summaryMd != null`, which is
   > *nullishness*, while the ternary at `:152` branches on **truthiness**: `summaryMd: ''` takes arm B
   > in the code and would be reported `vault-filename`. Two copies of one condition that disagree on
   > the empty string — the same read-to-the-end defect, inside the fix for a read-to-the-end defect). `sync-run.ts`'s
   > explicit branch then says either *"rename `<vault file>`"* or *"the cloud key `<key>` cannot be
   > relocated; it is unservable and has no local counterpart to rename."* **Behavior 26d gains a second
   > row for arm B**, or the `metadata-failed` mutation stays green against half the branch set.
   >
   > > **LEAD on how this survived, carried forward because it is the most useful part.** The round-14
   > > and round-16 Claude reviews **both** quoted `:152-154` — the full ternary, all three lines — and
   > > both glossed it as *"the vault filename"*. v19 encoded that reading in three places. **Two
   > > reviewers and the coordinator pasted the code and read past the `:`.** This project already has
   > > the rule *"quote the code, don't characterise it"*; the code was quoted. **Pasting is not
   > > reading, and a ternary is the cheapest place for a second branch to hide.**
   ...
   > **`{ ok: false; reason: 'unservable-base'; key: string }`**. Verified that a new variant works
   > mechanically: generic throw → caught per-video at `:812` → no baseline → re-fires cleanly, not stuck.
   >
   > **It must also name the manual repair, which behavior 26d did not require.** Behavior 26 demands
   ...
   >
   > **⛔ Round-16 M1 (BOTH halves, independently) — those two sentences cannot both be true, and v18
   > shipped them nine lines apart.** v18 said the variant needs **"no change to the caller"** because
   > the generic tail interpolates `key`:
   >
   > ```ts
   > // sync-run.ts:754-756 — what the operator actually sees
   > : `base reconciliation failed for ${id}: ${rec.reason}${'key' in rec ? ` at ${rec.key}` : ''}`);
   > ```
   >
   > That yields `base reconciliation failed for <id>: unservable-base at 003_bad.md`. It names the key
   > and says **nothing** about the key being a *local vault filename*, or what to do — which is the one
   > thing 26d requires. **Resolved in favour of the message:** `sync-run.ts` gets an explicit
   > `unservable-base` branch stating that the cloud video is blocked by a local vault filename and that
   > the file must be renamed to a servable single component before sync can relocate the base. **The
   > "no change to the caller" claim is DELETED** — it was true of the mechanics and false of the
   > requirement, and this is the third consecutive version to ship a rule contradicted elsewhere in the
   > same document (round-12 Codex found it in §3.6.3, round-15 H2 in the placement count).
3. **The adopt path is guarded in the CALLER. §3.5.1b row 4 is the ONE place the location is stated —
   and v20 said that while stating it nowhere, which is round-18 M2.** *(Round-17 M2: v19 moved this guard and left three sentences naming the old site
   `sync-run.ts:236-238` — the location this same section calls unimplementable. That bullet had
   already been corrected twice, at round 13 and round 15. **The defect is not the stale line; it is
   that one location was restated in four places.** State it once, point at it three times.)* It
   applies **ONLY when the receiver is the CLOUD.** It is **not** redundant with the seam: its job is to
   make the refusal happen *before* `claimVideoSlot`'s **durable insert** at `:240`. Delete it and the
   additive refusal lands at `upsertVideo` (`:286`), by which time a bare receiver row exists **and**
   the MD blob has been staged and promoted (`:263-268`) — precisely the state `sync-run.ts:230-235`
   records as round-2 H-R2-1 and forbids. Round 15 traced the consequence: not data loss (run 2 goes
   two-sided → `transferClassA` → the seam refuses at `:432`), but behavior 26's *"no receiver row is
   created"* would be **false** and every run would accumulate a bare row plus one orphan blob.

   > ### ⛔ Round-16 B1, Blocking — the guard was DIRECTION-AGNOSTIC, and that is the OPPOSITE defect
   >
   > **Every previous Blocking in this document was a guard that did not reach far enough. This one
   > reaches too far**, and the distinction matters because the two have opposite fixes.
   >
   > `copyAdditiveVideo` is **one function called in two directions** — `runSync`'s additive branch
   > picks the receiver at `sync-run.ts:624-627`:
   >
   > ```ts
   ...
   > ```
   >
   > The guard at `:236-238` runs on every call, so it also fires on **cloud→local hydration**.
   >
   > **Failure scenario.** A cloud video already holds an unservable `summaryMd` — §3.5's own
   > reachability table names the producer, and the serve/download/PDF/dig routes all 409 on it *today*.
   > The local copy is then lost (second machine, fresh vault, user deleted the file), leaving the video
   > **cloud-only**. **Today sync hydrates it back**: the vault has no servability guard — correctly,
   > per §3.4 — so the user gets their paid summary back as a file. **With a direction-agnostic guard,
   > that refusal fires forever**, and since **v18 closes the share route in the same version**, the
   > result is a **paid summary unreachable through every product path**, with no in-product repair —
   > a re-serve cannot run, because the serve path refuses before it reserves.
   >
   > **It also contradicts a user decision.** Decision ① is *"the vault wins — local filenames keep
   > their Unicode"*, and §3.4's central thesis is that the local path never needed this guard and has
   > run without one, full of Korean filenames, for the app's entire life. **A guard that refuses to
   > write a name into the vault is the inverse of that decision.**
   >
   > **And the stated repair is wrong in that direction:** §3.5.2 says the refusal re-fires *"until a
   > human renames the vault file"*. On cloud→local **there is no vault file** — the offending name is a
   > cloud key, and "rename" is not an operation the user has. Exactly the mirror of the diagnosis
   > §3.5.1 already makes for `reconcileCloudBase`.
   >
   > **Why this is `branch-coverage` and NOT a third instance of round 15's shape.** The repaired test
   > is *can a redesign remove it?* — **no**. The two directions belong to `runSync`'s additive branch,
   > not to this design; any placement under any shape still has to answer *"which receiver?"*. A new
   > shape cannot delete the branch, only fail to mention it. **That is §3.6.1b's own class**, and
   > §3.6.1b is the section that predicts it.
   >
   > **The uncomfortable part, recorded rather than smoothed over:** v18's own §3.5.2 asked the
   ...
   > `sync-run.ts:626`** (round-17 M1: v19 said *"at `:624-627`"*, which admits a position after the
   > sender-side `readMdBody`, and that ambiguity made behavior 26f unfalsifiable — see below).
...
#### 3.5.1b EVERY PLACEMENT × EVERY PRODUCER OF THE VALUE IT GUARDS

> ### ⛔ REWRITTEN AT ROUND 17, AND THE REWRITE *IS* THE ESCALATION
>
> v19's version of this table asked *"which side is the receiver?"* and answered it for all seven rows.
> **Round 17 then found a third instance of the very defect the table was built to pre-empt** — in row
> 3, which the table called *"One branch"* and which the **other** reviewer independently verified as
> one branch. Both verifications were correct **about the direction**: `cloud: cloudSide` really is
> hard-wired at `sync-run.ts:731`. The **name being tested** was not.
>
> **The branch question is about the OPERAND, not the CALL SITE.** Direction is one way a guarded value
> branches, and it is the conspicuous one because it appears in the signature. **Provenance is the one
> that hides**, because both arms produce the same *type* and the ternary sits upstream of the guard.
>
> So the columns changed: every row now names **the value guarded** and **every producer of it**. The
> escalation fired on round 17 and this is what it bought — the instrument was rebuilt, not the design.

**The discriminator each row must answer:** is this placement **provenance-blind** (every producer is
refused identically, so the count cannot matter) or **provenance-dependent** (the justification, the
message, or the correctness of refusing differs by arm)? **Row 3's defect was being provenance-dependent
while the table recorded it as blind.**

| # | Placement | Value guarded — **cited at its DEFINING line** | Producers of that value | Blind or dependent |
|---|---|---|---|---|
| 1 | **The metadata seam** — `videoDataPayload` | **PARAMETER** `v` (`lib/storage/supabase/supabase-metadata-store.ts:19`) | **Call sites, THREE** — `:119` `upsertVideo`, `:143` `updateVideoFields`, `:160` `bulkUpdateVideoFields` | **BLIND.** Refusing identically regardless of origin *is the seam's entire purpose*, and §3.5.1's claim that the producer count stops mattering holds **here and nowhere else**. ⚠ *A BLIND row must never be asked to let one producer through — round-18 B1* |
| 2 | **The mint** | `baseName` (`lib/job-queue/summary-handler.ts:96`) | **One** — `${padSerial(serial)}_${slugify(payload.title)}`, no branch in the expression | BLIND (vacuously) |
| 3 | **`reconcileCloudBase`** (in memory) | `to` (`lib/cloud-sync/reconcile-serial.ts:152`) — aliased to `newBase` at `:186`; **the alias is not the definition** | **TWO** — arm A `baseOf(local.summaryMd)`, a *vault filename*; arm B `baseOf(applySerial(cloud.summaryMd, local.serial))`, the *cloud key renumbered* | **DEPENDENT** ← round-17 **B1**. Justification and repair message are true of arm A only |
| 4 | **The adopt** — ⛔ **THE GUARD LIVES AT `sync-run.ts:624-627`, ABOVE `:626`, ON THE `to = cloudSide` ARM. This row is the ONE normative statement of that location (round-17 M2, round-18 M2)** | `present` (`lib/cloud-sync/sync-run.ts:619`) — `.summaryMd` is the tested field | **TWO** — `(lv ?? cv)!`; provenance coincides with direction, `presentIsLocal` at `:620` | **DEPENDENT** ← round-16 **B1**. Cloud receiver only |
| 5 | **The seam refusal reaching `transferClassA`** | **PARAMETER** `winnerVideo` (`lib/cloud-sync/sync-run.ts:372`) — `key = winnerVideo.summaryMd` at `:379` | **Call sites, TWO** — `:782` `copyToCloud` passes `(localSide, cloudSide, lv)`, `:793` `copyToLocal` passes `(cloudSide, localSide, cv)` | **DEPENDENT** ← **26c3**, round 15. Cloud loser only |
| 6 | **The share guard** | `mdKey` (`lib/share/serve.ts:47`) | **TWO** — `artifact?.key ?? vid.data.summaryMd`. Arm A is `artifacts.summaryMd.key`, taken FIRST; arm B is the top-level fallback. *(Round-18 M1: this cell previously claimed a single producer, named arm B alone, and cited the function signature at `:13` instead of the assignment.)* | **BLIND** — both arms are refused identically, and the guard tests exactly the value the serve path consumes |
| 7 | **`serialize`** — the write-time schema | **PARAMETER** `envelope` (`lib/html-doc/model-store.ts:34`) | **Call sites, THREE** — `generate.ts:50` (**local**), `serve-doc.ts:174` (cloud serve), `sync-run.ts:464` (ship) | **BLIND, deliberately** — see below |

> **`scripts/check-producer-enumeration.py` CHECKS THIS TABLE, and it is the reason there is no fourth
> hand-built version.** It resolves each row's cited `path:line`, refuses a citation that does not
> declare the named value, refuses a **bare alias** (row 3's `newBase = d.to!` has no branch at the
> rename — the arms are two lines inside another function), and fails any row claiming **One** whose
> defining expression contains `?:`, `??`, `||`, `catch` or `switch`. Run it with `--self-test`: 11
> cases, including both measured misses.
>
> **What it does NOT check, and this is your manual obligation:** the three **PARAMETER** rows. Their
> producers are call sites, not arms of an expression, and the script reports them as
> `UNCHECKABLE-BY-DESIGN` rather than passing them. It also cannot see a producer split more than one
> call from the cited line. It catches the two shapes that were **measured**, which is the bar — not
> completeness.

**Row 7 is uniform ON PURPOSE, and that was verified rather than assumed.** The `videoId` requirement
binds the **local** generate path too, so the question is whether local can satisfy it. It can:
`runHtmlDoc(videoId: string, …)` takes it as its **first parameter** (`generate.ts:12`), with `video`
resolved from the index at `:23`. A model envelope means the same thing on both sides, and an envelope
that cannot say which video it belongs to is exactly the defect round-13 H1 opened. Rows 4 and 5 are
dependent for the opposite reason: *servability of a cloud key is meaningless in the vault* — decision ①.

> **THE RULE, restated against the operand.** A placement is not specified until you have named **the
> value it guards and every producer of that value** — including when the answer is *"many, and they are
> all refused identically"*, which is a real and strong answer (row 1) rather than a missing one.
>
> **Ask it in this order**, because the cheap questions hide the expensive one:
> 1. *What value does this guard test?* — name it, in the table.
> 2. *Where does that value come from?* Follow it to **every** producer. **A ternary is two producers.**
> 3. *Does the justification, the message, or the correctness of refusing differ per producer?* If yes,
>    the placement is **dependent** and every arm needs its own stated outcome.
>
> **Three instances, three rounds, one shape** — 26c3 (r15), B1 (r16), B1 (r17). The first two were
> direction defects, which is why v19's table asked about direction and felt complete. **A prophylactic
> that asks something adjacent to the defect class returns a confident, verifiable, useless answer.**
...
|---|---|---|---|
| `copyAdditiveVideo` — **receiver is the CLOUD** (`copyToCloud`) | **the adopt guard — see §3.5.1b row 4** (in the caller, above the sender-side read) | **nothing** | Video stays one-sided. Throw → caught per-video at `:812` → `report.errors`, **no `writeVideoBaseline`** → re-fires identically every run **until a human renames the vault file** — which exists, and is the thing to rename. ✅ intended |
| `copyAdditiveVideo` — **receiver is the VAULT** (`copyToLocal`) | **NOWHERE — the guard does not apply** (round-16 B1) | n/a | **Hydration SUCCEEDS**, exactly as today. §3.4 argues at length that the local path never needed this guard; decision ① says the vault wins. Refusing here would strand a paid cloud artifact whose every other route is already 409 or closed by this version, and the repair sentence would name a vault file **that does not exist** |
...
| `reconcileCloudBase` | **in memory**, before the copy phase | **nothing** | Returns `unservable-base` → the **explicit `sync-run.ts` branch** (§3.5.1 placement 2; round-17 L1: this cell said *"generic throw"*, the residue of the claim round-16 M1 deleted) → `:812` → no baseline → re-fires. ✅ clean |
...
```ts
// lib/html-doc/model-store.ts:34 — the only path from an envelope VALUE to bytes
...
> **⚠ Round-16 L2 — "the only path from an envelope to bytes" is this document's FOURTH falsified
> universal.** §3.4 already lists three. Two production writers put bytes into a `models/<base>.json`
> object **without** calling `serialize`, and round 16 found them by enumeration — the Codex half graded
> the first **Blocking** and escalated to REDESIGN; the Claude half found **both** and graded them Low.
> The Low is correct, and the coordinator reached that reading independently before either half
> reported:
>
> | Writer | Evidence | Constructs an envelope? |
> |---|---|---|
> | cloud base relocation — **byte copy** | `reconcile-serial.ts:98` (`MODEL_KEY(base)` ∈ `paidKeysUnder`), `:118` (`remap`), `:282` `cloud.blob.copy` → `blob-store.ts:155` `store.put(p, to, src.bytes, …)` | **No** — moves bytes verbatim |
> | local serial migration — **direct `fs.writeFileSync`** | `serial-migrate-exec.ts:141`, via `serial-provenance.ts:14-18` (`JSON.parse` → set `sourceMd` → `JSON.stringify`) | **No** — a field-preserving round-trip that bypasses the BlobStore seam entirely |
>
> **Neither can violate the invariant, and the reason is worth stating precisely: a COPY HAS NO AUTHOR
> TO DEMAND `videoId` FROM.** Both are *transforms of an already-conforming envelope*, and both
> preserve unknown fields, so neither can produce an envelope lacking `videoId` that did not already
> lack one — a relocated legacy envelope **propagates** a legacy state rather than **introducing** one,
> and lands on the same legacy branch it was already on. The property the design needs is intact.
>
> **The sentence asserting HOW it is intact was wrong, and that is the whole defect** — it is the
> sentence a future reviewer trusts instead of re-deriving. The precise claim: **`serialize` is the only
> path from a `ModelEnvelope` *value* to bytes. A writer that BUILDS an envelope cannot avoid it; a
> writer that COPIES one cannot break it.**
>
> **The mutation gap was real** and is closed by behavior **18j7**: round-16 Codex correctly noted that
> every listed mutation leaves `18j5` green while `copyBlob` creates a model object with no `videoId`.
...
> **⚠ ROLLOUT COST — round-17 L2, and the precedent for stating it is in this document.** Making
> `videoId` required on `serialize`'s parameter type breaks **every** call site that builds an envelope
> literal. Enumerated over the repo, production **and** test:
>
> | Location | Call sites | Distinct literals/fixtures to edit |
> |---|---|---|
> | `generate.ts:50`, `serve-doc.ts:174`, `sync-run.ts:464` | 3 | 3 (production — all three have the id in scope, verified) |
> | `tests/lib/html-doc/rerender.test.ts` | 14 | 2 (the `envelope()` helper + the literal at `:210`) |
...
> | **total** | **41** | **~20** |
>
> §3.5 already states exactly this class of cost for the *other* seam change (round-12 Low, *"the
> rollout is wider than three adapters"*). **A `tsc` error in 41 places is not a defect; it is a
> predictable interruption, and the plan carries it or the plan is wrong.** Two facts in the design's
> favour, both verified: `companionTransfer` has `winnerVideo.id` in scope (`sync-run.ts:445`), so
> 18j6's stamp compiles; and because `decideCompanion` returns a **read** `ModelEnvelope`, `tsc` will
> **force** the stamp at `:464` rather than merely inviting it.
...
| 18j8 | **After a LOCAL serial migration the rewritten envelope still carries `videoId`** — `rewriteEnvelopeSourceMd` (`serial-provenance.ts:13-17`, written at `serial-migrate-exec.ts:141`) is a `JSON.parse` → set one field → `JSON.stringify` round-trip, and its unknown-field preservation is the whole reason it is a safe `serialize` bypass. *(Round-17 L4: the mutation row named this half and no behavior observed it, so mutating it left 18j7 green.)* | unit |
| 18j7 | **After a cloud base relocation the copied envelope still carries the SAME `videoId`** — the byte-copy at `reconcile-serial.ts:282` bypasses `serialize`, so the invariant is preserved by the *copy*, not by the write schema. 18j3 asserts the ship still succeeds, which is the consequence; this asserts the credential (round-16 L2) | integration |
...
| 26f | **The adopt guard runs ABOVE `sync-run.ts:626`** — asserted by **no `get` on the SENDER's blob store for the summary key** when the refusal fires. *(Round-17 M1: v19 asserted "no `ensureReceiverSlot` call", which **does not discriminate** — `ensureReceiverSlot` is module-private and unspyable, and its only observable, `claimVideoSlot` at `:214`, is equally absent under the mutant, because the old placement at `:236-238` is itself above `:240`. The sender-side `readMdBody` at `:626` is the one observable the two placements differ on.)* | integration |
...
| 26d | **ARM A — a relocation onto an unservable VAULT name is refused IN MEMORY**, and only when the **old base was servable**: no blob is copied, the old base is **intact**, nothing is deleted, the result is `{ ok:false, reason:'unservable-base', key, origin:'vault-filename' }`, **and the message names the file to rename** (round-14 B1, round-15 M3, round-17 B1) | integration |
| 26d2 | **unservable → unservable ⇒ SKIPPED, not relocated**: `{ ok:true, action:'skipped-unservable' }`, **zero `copy` calls**, **no seam write**, the divergence appears in `report.errors`, and `reconcileClassA` → `copyToLocal` then hydrates the paid summary into the vault. *(v20 asserted the relocation PROCEEDS, which is unsatisfiable against a real `SupabaseMetadataStore`: the seam throws on the row write — round-18 **B1**.)* | integration |
| 26d4 | **unservable → SERVABLE ⇒ RELOCATES** — a genuine repair, and the seam accepts it. **No behavior covered this cell before round 18**, so nothing would have caught a fix that skipped it too | integration |
| 26d3 | **ARM B still refuses when the OLD base was servable** — a servable cloud key whose renumbering by `applySerial` pushes it past 131 code points — and the message says the cloud key has **no local counterpart to rename**, `origin:'cloud-key'` | integration |
...
| Strip unknown fields in the **relocation copy** | **18j7** — the `serialize` bypasses are safe only because they PRESERVE unknown fields; nothing asserted that until round 16 | PROVISIONAL |
| Strip unknown fields in `rewriteEnvelopeSourceMd`'s JSON round-trip | **18j8** | PROVISIONAL |
...
| **Drop the `isServableSummaryKey(oldBase)` conjunct** — refuse whenever the NEW base is unservable | **26d2** — the round-17 B1 Blocking, reproduced. Goes green against 26d alone | PROVISIONAL |
| **Return `{ok:true}` and RELOCATE on unservable→unservable instead of SKIPPING** | **26d2** — the round-18 B1 Blocking: the copies run, then the seam refuses the row. Goes green against any assertion that only checks the return value | PROVISIONAL |
| Skip on unservable→**servable** as well | **26d4** — the repair cell stops repairing | PROVISIONAL |
| Make the skip SILENT (omit the `report.errors` entry) | **26d2** — a divergence that no longer heals must remain visible | PROVISIONAL |
| Drop the `origin` field and emit one message for both arms | **26d3** — arm B is told to rename a vault file that does not exist | PROVISIONAL |
| Widen the refusal so a servable→over-length renumbering is allowed through | **26d3** — the one case arm B must still refuse | PROVISIONAL |
