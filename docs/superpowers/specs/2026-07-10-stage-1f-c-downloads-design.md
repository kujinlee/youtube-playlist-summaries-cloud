...
- **Share HTML serve** (`app/s/[token]/route.ts`): token → `getShareServeContext` (confused-deputy guard) → get-only `service_role` read of `mdKey` (`:37`) → parse → `readTitleStableModel` (serve-if-title-stable, tolerating generator-version skew since 2026-08-11; **never charges**) → share-mode render → `no-store` + `Referrer-Policy: no-referrer`.
...
| D5 | **HTML download reuses each caller's existing money path verbatim:** owner → `resolveMagazineModel` (materialize + charge once, cached, exactly like a view); share → `readTitleStableModel` (serve-if-title-stable, else "not ready", **never charges**). | Downloading HTML is materially identical to viewing it; the only delta is the disposition header. All 1F-a/1F-b money invariants hold. |
...
| C10 | Share HTML not-ready | share `format=html`, model absent / titles drifted / schema-invalid *(no longer merely version-skewed — see 1F-b B8, 2026-08-11)* | **503** "not ready" (1F-b path); no charge |
