...
**Date:** 2026-07-08
**Status:** Draft **v6** — hardened across five dual adversarial rounds (r1: Codex `task-mrclxoks`; r2: `task-mrcme452`; r3: `task-mrcmvol7`; r4: `task-mrcnpezu`; r5: `task-mrco7xj2` — each + a Claude Opus reviewer; re-reviews in `docs/reviews/spec-stage-1d-v{2,3,4,5}-rereview.md`).
**v6 closes the round-5 Blocking + three High — the last cost-leakage vectors:**
...
1. **Sound cap = one-run worst-case est × bounded attempts + bounded duration + never-release (fixes round-1 Blocking-1 *and* round-2 B-A).** A cap only bounds money if `reserved ≥ actual` for the **whole lifetime** of every job row — including its durable retries. The soundness theorem:
   ...
   - **(c) enforced token caps (round-3/4/5 fix)** — every cloud Gemini call carries a `maxOutputTokens` cap **and `thinkingConfig.thinkingBudget: 0`** (so thinking tokens, billed at the output rate, are eliminated — not merely output-capped; round-5 H-new-1); the transcript is **byte-truncated over the *rendered* `buildIndexedTranscript(kept)` string** before the summary/quick-view prompts (proof: a Gemini token spans ≥ 1 byte ⇒ billed tokens ≤ bytes ≤ `MAX_TRANSCRIPT_INPUT_BYTES`, and the per-line `[i @m:ss]` prefixes are inside the measured string — round-5 B2); and the video-transcription **input** is hard-capped by a **`countTokens` preflight** (identical LOW-res config, VOD-only) that rejects over-budget before any billed `generateContent` (round-4 B3 / round-5 H-new-2). The **priced model is pinned/asserted** (`gemini-2.5-flash`, flat-rate) so an env override can't inflate cost past CI (round-5 H-new-3). So every token term is code-enforced: transcription in (`countTokens`) & out (`maxOutputTokens`, no thinking), summary/quick-view in (byte cap) & out (`maxOutputTokens`, no thinking). Caps are cloud-scoped options threaded via `CloudGeminiCaps` (§9); the local pipeline passes none and is unchanged. *(Residual: the `countTokens` feasibility for YouTube `fileData` is impl-verified — if unsupported, the video-input term falls back to duration×rate-assumed and is flagged; §3 L1.)*
...
- Prices — `PRICE_IN_PER_1M_CENTS` = 30, `PRICE_OUT_PER_1M_CENTS` = 250, **dated/sourced** (`gemini-2.5-flash`, flat-rate, 2026-07 pricing; no long-context tier). The **model is pinned/asserted** to the priced constant (round-5 H-new-3) — a deploy-time check + the guard test assert `active model == priced model`; changing prices/model requires re-deriving `est` (round-5 Codex-B1-residual).

Caps are **generous** — they never truncate real ≤30-min speech content (≈8k tokens ≪ 40 960 bytes / 32 768 out), only the pathological/adversarial case. One-run worst case at 30 min, `gemini-2.5-flash` list price ($0.30/1M in, $2.50/1M out), thinking off, **every pass at max input+output**:
- **Transcription** (`TRANSCRIBE_MAX_PASSES`=3): 3 × ((300 000+4 000)×$0.30/1M + 32 768×$2.50/1M) ≈ 3 × ($0.091+$0.082) ≈ **$0.52**.
- **Summary loop** (`SUMMARY_MAX_PASSES`=12): 12 × ((40 960+4 000)×$0.30/1M + 8 192×$2.50/1M) ≈ 12 × ($0.013+$0.020) ≈ **$0.40**. *(byte-cap used as the input-token bound.)*
- **`extractQuickView`** (`QUICKVIEW_MAX_PASSES`=3): 3 × ($0.013+$0.020) ≈ **$0.10**. *(input bounded by the summary output ≤ `MAX_SUMMARY_OUTPUT_TOKENS`; the byte cap is used as a safe over-estimate — round-5 L.)*
- One-run worst case ≈ **$1.03**. **`$1.25` (125¢) is set as the upper bound** (≈21% margin).

**`summary_est_cents`, `max_duration_seconds`, `summary_max_attempts`, the code token caps, the pass-count constants, the prices, AND the pinned model are a coupled set** — changing any requires re-deriving and raising `est`. The §8 guard test recomputes `per_run_worst` from the **live** `max_duration_seconds` **and every imported code constant** (caps, pass counts, overhead, prices) × the retry budget, asserts `summary_est_cents ≥ per_run_worst × summary_max_attempts`, **and asserts the active model == the priced model** — so DB drift (`UPDATE`), code drift (a token cap, retry count, `MAX_SUMMARY_ATTEMPTS`, price), and env-model drift all fail CI. At $1.25/$5 the cap admits ~4 summary jobs/day globally.

**L1 — the one non-code-capped input:** the video-transcription input cannot be capped a priori (you send a URL). The **runtime `countTokens` preflight** (LOW-res, VOD-only) is the hard ceiling. Its soundness depends on two impl-verified facts: (a) `countTokens` resolves a YouTube `fileData` request (not just the URL string); (b) it uses the *same* LOW-res `generationConfig` as the billed call. If (a) fails, the term reverts to duration×rate-assumed — **flag for the user**; VOD-only (reject live/upcoming) closes the mutable-URL TOCTOU (round-5 Codex-B3).
...
2. New row only → M-D duration backstop (robust cast; reject-not-admit for malformed — round-3 M3-1, round-4 M1):
     ...
     if v_dur is null or v_dur !~ '^[0-9]{1,7}(\.[0-9]{1,6})?$'  -- missing/non-numeric/over-long ⇒ reject. Length-bounded (≤7 int digits = ≤9,999,999s ≫ any real duration) so a 200k-digit decimal can't reach ::numeric and raise a precision error before PJ003 (round-5 Codex-M1). Exponent form (1e21) also misses the regex ⇒ reject (fail-closed).
        or floor(v_dur::numeric) > v_cfg.max_duration_seconds    -- NUMERIC compare, NO ::int (round-4 M1)
...
```
- **`gemini.ts`** — add an **optional** `caps` param to `transcribeViaGemini`, `generateSummary`, `extractQuickView`; when present, set `generationConfig.maxOutputTokens` **and `generationConfig.thinkingConfig = { thinkingBudget: 0 }`** (untyped passthrough — the SDK spreads `generationConfig` into the request body; same technique the code already uses for `mediaResolution`, `gemini.ts:515`). Add a `countTokens` preflight inside `transcribeViaGemini` that builds the request with the **same LOW-res `generationConfig`**, calls `model.countTokens`, and throws `NonRetryableError` when `totalTokens > transcribeInputTokens` (bounded by a timeout; note the double fetch — round-5 M-new-2). **Pin/assert the model** = the priced `gemini-2.5-flash`. Export the sizing constants, **pass-count constants** (`*_MAX_PASSES` / `MAX_SUMMARY_ATTEMPTS` + `retries`), and **dated price constants** for the §8 guard test. *(Impl gate: verify `countTokens` resolves a YouTube `fileData` request; if not, fall back to the duration×rate bound and flag the user — §3 L1.)*
...
- **`summary-core.ts`** — `summaryCore`'s `opts` gains an optional `caps: CloudGeminiCaps`, forwarded to **all three** injected deps. The transcript truncation — **drop whole trailing segments until `buildIndexedTranscript(kept)` UTF-8 byte length ≤ `transcriptInputBytes`**, the same `kept` list feeding the prompt and `resolveTranscriptTokens` (so `[[TS:n]]` stays in range) — happens here or in the injected wrapper.
...
7. **Tunable defaults** (§3 seeds): registered 20 summary + 5 dig/mo, anon 2 summary/mo; `$5/day`; `$1.25`/kind one-run worst-case; `1` attempt/kind; 30-min duration; VOD-only; N=100; queue 200; velocity 15/IP/hr; CAPTCHA soft 5. Code constants (§9, deploy+guard-test, not `UPDATE`): `MAX_TRANSCRIBE_INPUT_TOKENS`=300000 (`countTokens` reject; ~142 tok/s observed ⇒ ~256k real ≤ 300k), `MAX_TRANSCRIBE_OUTPUT_TOKENS`=32768, `MAX_TRANSCRIPT_INPUT_BYTES`=40960 (rendered), `MAX_SUMMARY_OUTPUT_TOKENS`=8192, `thinkingBudget`=0, `PROMPT_SCHEMA_OVERHEAD_TOKENS`=4000, pass counts 3/12/3, prices 30/250 ¢-per-1M (dated `gemini-2.5-flash` 2026-07), model pinned/asserted.
