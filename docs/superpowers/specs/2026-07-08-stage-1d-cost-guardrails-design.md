...
**Date:** 2026-07-08
**Status:** **v7 — CONVERGED** across seven dual adversarial rounds (r1–r7; Codex `task-mrclxoks`/`mrcme452`/`mrcmvol7`/`mrcnpezu`/`mrco7xj2`/`mrcopnpr`/`mrcp6dvw` + a Claude Opus reviewer each; re-reviews in `docs/reviews/spec-stage-1d-v{2,3,4,5,6}-rereview.md`). **Round-7: both reviewers returned no new Blocking/High** (Claude explicitly "converged"; Codex zero new Blocking/High) → gate met. Two round-7 closeout tightenings applied below (thinking gate `thoughtsTokenCount === 0`, retry-count exported constants); known-accepted residuals recorded in §10/§11. Pending human approval.
...
- Pass-count constants (exported): `TRANSCRIBE_MAX_PASSES` = `TRANSCRIBE_RETRIES`+1 = 3, `SUMMARY_MAX_PASSES` = `MAX_SUMMARY_ATTEMPTS`(4) × (`GENERATE_JSON_RETRIES`+1) = 12, `QUICKVIEW_MAX_PASSES` = `GENERATE_JSON_RETRIES`+1 = 3. **The retry defaults `TRANSCRIBE_RETRIES` and `GENERATE_JSON_RETRIES` must be single exported constants that ARE the actual default-parameter values in `transcribeViaGemini`/`generateJson` AND the values these derivations multiply** (round-7 Claude M1) — otherwise a one-line `retries=2→3` bump raises real passes with green CI. The guard test imports the `*_MAX_PASSES` (which thus track the retry defaults).
...
| **Impl-verification gates** (one-time, live/recorded) | (1) **thinking honored** — a representative cloud transcribe+summary with `thinkingBudget: 0` returns `usageMetadata.thoughtsTokenCount` **present and `=== 0`** (round-7 Codex): an **absent** field is *not* proof (can't distinguish "no thoughts billed" from "not reported") → treat absence as **unverified → fail startup/flag**, do not pass. (2) **`countTokens` on YouTube `fileData`** — resolves the video's tokens (not the URL string) at LOW res; **if not, the video-transcription fallback is disabled** (caption-less → reject) so the bound stays sound (round-6 H-r6 / §2 c′). |
...
```
- **`gemini.ts`** — add an **optional** `caps` param to `transcribeViaGemini`, `generateSummary`, `extractQuickView`; when present, set `generationConfig.maxOutputTokens` **and `generationConfig.thinkingConfig = { thinkingBudget: 0 }`** (untyped passthrough — the SDK spreads `generationConfig` into the request body; same technique as `mediaResolution`, `gemini.ts:515`). Add a `countTokens` preflight inside `transcribeViaGemini` that builds the request with the **same LOW-res `generationConfig`**, calls `model.countTokens`, and throws `NonRetryableError` when `totalTokens > transcribeInputTokens` (timeout-bounded; double fetch noted). **If the impl-verification finds `countTokens` can't resolve YouTube `fileData`, disable the transcribe fallback** (caption-less → `NonRetryableError`) rather than proceeding unbounded (§2 c′). **Export the *resolved* model constants** (`SUMMARY_MODEL`/`TRANSCRIBE_MODEL`, post-`??`), the **retry-default constants** (`TRANSCRIBE_RETRIES`/`GENERATE_JSON_RETRIES` — the actual signature defaults, feeding `*_MAX_PASSES`), the sizing + pass-count constants, and the **dated price constants (incl. audio price + `AUDIO_TOKENS_PER_SEC`)** for the §8 guard test; assert `resolved model == priced model` at handler init.
...
- **Known-accepted residuals at convergence (round-7):** (a) *`countTokens` per-request faithfulness* — the preflight is a one-time-verified proxy for what `generateContent` bills; true closure is the deferred true-reconcile (owner: 1D-followup) — the fail-closed gate is the accepted 1D mitigation (round-7 Claude M2). (b) *thinking honored-gate is one-time*, not per-call; a silently-ignored budget on an atypical input is an accepted demo-stage residual (round-7 L1). (c) *handler `MAX_DURATION_SECONDS` lowering* is defense-in-depth, **not** load-bearing for the audio bound (enqueue PJ003 already guarantees `duration ≤ 1800` ⇒ `audio ≤ 57 600` for every enqueued job); still implemented in 1D per above (round-7 L2). (d) PJ003 rejects >6-fractional-digit durations — harmless (YouTube sends integers).
