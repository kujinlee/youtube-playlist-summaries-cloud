...
⚠ WHAT IT DOES NOT DO, STATED SO THE NEXT READER DOES NOT HAVE TO REDERIVE IT:
 ...
 2. **The other twelve rules have no executable form** — identity, statistics, derived counts,
    filters, physical type properties. They are printed as UNVERIFIED-BY-EXECUTION with their class,
    every run. Silence about them would read as coverage.
 3. It proves a reason is NOT FALSE in the direction tested. It cannot prove the exclusion is wise.
"""
...
UNVERIFIABLE: tuple[tuple[str, str], ...] = (
    ("rules 1, 2, 15", "IDENTITY — an OID, a namespace/owner reference, or the object's own name. "
                       "The name IS the manifest key, so digesting it would be circular."),
    ...
    ("rule 8", "A FILTER, not a property — CATALOG_SQL's WHERE clauses use these to decide which "
               "rows are M4 objects at all, so the value is fixed for every row that reaches a digest."),
    ("rules 9, 10, 12, 13", "PHYSICAL / IRRELEVANT — fast-default machinery, foreign-table options, "
                            "type I/O properties, replica identity, C-language paths. M4 ships no C "
                            "functions and no foreign tables."),
...
def self_test() -> int:
    ...
    check("the rule count has not drifted from what this file was written against",
          len(cc.EXCLUDED), 18)
    covered = claimed | {n for label, _ in UNVERIFIABLE
                         for n in [int(t.strip(",")) for t in label.split()
                                   if t.strip(",").isdigit()]}
    check("EVERY rule is either executed or explicitly listed as unverifiable",
...
def main() -> int:
    ...
    if false_reasons:
        ...
        return 1
    print(f"\n✅ {len(CLAIMS)} executable reason(s) EXECUTED and held; "
          f"{len(UNVERIFIABLE)} rule group(s) have no executable form and are named above.")
    return 0
