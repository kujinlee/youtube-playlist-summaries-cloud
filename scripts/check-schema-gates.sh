...
run "1/8  schema + assertions (verify-schema.sh)"      "$SPEC/verify-schema.sh"

# 2. Every guard is mutation-checked — a guard that is never mutated is documentation.
run "2/8  mutation suite (mutate-schema.py)"           "$SPEC/mutate-schema.py"
...
run "3/8  guard coverage (check-guard-coverage.py)"    ./scripts/check-guard-coverage.py
...
run "4/8  sentinel meanings (one NULL, one meaning)"   ./scripts/check-sentinel-meanings.py
run "5/8  vocabulary collisions (one mechanism)"       ./scripts/check-vocabulary-collisions.py

# 6. Docs integrity — cheap, and catches spec/prose drift.
run "6/8  documentation integrity"                     python3 scripts/check-docs.py
...
# 7. Is the manifest the gate trusts still what the schema produces? Without this the gate can be
...
run "8/8  live catalog matches M4_PHASE=${M4_PHASE:-pre}" \
